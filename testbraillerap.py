...
def get_parameter_fname ():
    paramfname = "test_brap__parameters.json"
    if detected_os == KnownOS.Linux:
        home = Path.home ()
        dir = Path.joinpath(home, ".testbraillerap/")
        print (home, dir)
...
if __name__ == "__main__":
    ...
    if detected_os == KnownOS.RPI:    
        window = webview.create_window(
            APP_NAME, entry, js_api=api, focus=True
        )
    else:
        window = webview.create_window(
            APP_NAME, entry, js_api=api, focus=True, maximized=True
        )
